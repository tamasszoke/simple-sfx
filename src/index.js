...
const sfx = (audioList, devMode) => {
    ...
    if (audioList && audioList.length > 0) {
      try {
        for (let i = 0; i < audioList.length; i++) {
          ...
          devMode && console.log('Audio loaded: ' + audioName)
        }
        devMode && console.log('All audio loaded, total items: ' + audio.length)
      } catch (error) {
  ...
  const play = (settings) => {
    if (settings) {
      if (audioOn === true) {
            ...
            if (audio[i].name === settings.name) {
              if (settings.loop) { // loop playing
                try {
                  ...
                  audio[i].data.play()
                  devMode && console.log('Playing audio: ' + settings.name)
                  return 2
                } catch (error) {
        ...
        devMode && console.log('Audio disabled, not playing! ' + settings.name)
      }
    } else {
            ...
            for (let i = 0; i < audioNames.length; i++) {
              for (let j = 0; j < audio.length; j++) {
                if (audio[j].name === audioNames[i] && !audio[j].data.paused) {
                  ...
                  devMode && console.log('Audio stopped: ' + audioNames[i])
                  return 1
  ...
  const stopAll = () => {
    if (audioOn === true) {
      try {
        ...
        devMode && console.log('All audio stopped.')
      } catch (error) {
  ...
  const on = () => {
    audioOn = true
    devMode && console.log('Audio enabled.')
    return 1
  }
  ...
  const off = () => {
    try {
      stopAll()
      audioOn = false
      devMode && console.log('Audio disabled.')
      return 1
    } catch (error) {
